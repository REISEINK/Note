...
{
	// 把重力方向转换到模拟空间
	SimSpaceGravityDirection = TransformWorldVectorToSimSpace(Output, FVector(0.f, 0.f, -1.f));

	// 把外力转换到模拟空间
	FVector OrientedExternalForce = ExternalForce;
	if (!OrientedExternalForce.IsNearlyZero())
	{
		OrientedExternalForce = TransformWolrdVectorToSimSpace(Output, OrientedExternalForce);
	}
